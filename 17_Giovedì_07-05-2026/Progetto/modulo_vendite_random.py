    ...
    def __init__(self):
        ...
        def creazione_casuale(self):
            ...
            while True:
                ...
                match scelta:
                    case "1":
                        codice, tessuto, colore, taglia, tipo_taglio, numero_bottoni, è_reversibile = random_caratteristiche_capo_principale(codice, self._lista_tessuti, self._lista_colori, self._lista_taglie, self._lista_tipo_taglio)
                        ...
                        if capo_principale_scelto == "Giacca":
                            ...
                            self._lista_catalogo.append(capo_principale)
                    case "2":
                        codice, tessuto, colore, larghezza, tipo_chiusura, ha_piega_decorativa = random_caratteristiche_finitura(codice, self._lista_tessuti, self._lista_colori, self._lista_larghezze, self._lista_tipo_chiusura)
                        finitura_scelta = random.choice(self._lista_finitura)

                        if finitura_scelta == "Cravatta":
                            finitura = Cravatta(codice, "Cravatta", tessuto, colore, larghezza, self._prezzo_cravatta)
